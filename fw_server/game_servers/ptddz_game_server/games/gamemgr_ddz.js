...
var db = require('../../externals/utils/db');
...
function doGameOver(game, userId, dissolve) {
    ...
    var fnNoticeResult = function (isEnd) {
        ...
        userMgr.broacastInRoom('ddz_over_push', resultData, userId, true);
    ...
    if (game != null) {
        ...
        for (var i = 0; i < roomInfo.seats.length; ++i) {
            ...
            db.add_games_and_scores(sd.userId,sd.score);
    ...
    if (dissolve || game == null) {
        ...
        store_game(game, function (ret) {
            ...
            if (roomInfo.numOfGames == 1 && !dissolve) {
                // db.cost_gems(game.gameSeats[0].userId,game.conf.cost);
                if (roomInfo.conf.srzf) {
                    for (var i = 0; i < game.gameSeats.length; i++) {
                        db.cost_gems(game.gameSeats[i].userId, roomInfo.conf.cost);
                    }
                }
                else if (!roomInfo.conf.dkfj) {
                    // 非代开房间, 扣除房主的房卡
                    db.cost_gems(game.gameSeats[0].userId, roomInfo.conf.cost);
                }
            }else if(dissolve){
                 //代开房间第一局未结算直接申请解散，返还开房者房卡
                // console.log('roomInfo',roomInfo);
                if (roomInfo.numOfGames == 1) {
                   if(roomInfo.conf.dkfj){
                        // // 代开房间, 扣除开房者的房卡
                        console.log('代开房间第一局申请解散返回开房者钻石',roomInfo.conf.creator);
                        if(roomInfo.conf.creator){
                            db.add_user_gems(roomInfo.conf.creator, roomInfo.conf.cost,null,false);
                        }else{
...
exports.setReady = function (userId, isSync) {
    var roomId = roomMgr.getUserRoom(userId);
    if (roomId == null) {
        return;
    }
    ...
    if(isSync && !(_.isUndefined(game) || game === null)){
        ...
        // console.log("Log_setready:",game.state);
        // console.log("Log_ddz_sync_push:",data);
...
function sendJiaoFenMsg(curUserId, nextUserId, jiaofenmodel, score, game) {
    ...
    userMgr.broacastInRoom('ddz_jiaofen_push', jiaofen_data, curUserId, true);
}
//发送确认地主身份消息
function sendDizhuMsg(dizhu_uid, game) {
    ...
    if (game.conf['gamemodel'] !== GAME_MODEL[0]) {
        dealLZ(game);
        //通知癞子牌
        userMgr.broacastInRoom('ddz_laizi_push', {laizi: game.laiziPaiArr}, dizhu_uid, true);
    }
    //通知叫地主结果
    userMgr.broacastInRoom('ddz_dizhu_push', {dizhu_uid: dizhu_uid}, dizhu_uid, true);
...
function sendChuPaiMsg(curUserId, nextUserId, lastPaiData, paiArr, left_pai_cnt, game) {
    ...
    //通知出牌
    userMgr.broacastInRoom('ddz_chupai_push', chupai_data, curUserId, true);
}
...
exports.syncLocation = function (userId) {
    ...
    if (data.length >= 2) {
        userMgr.broacastInRoom('location_push', data, userId, true);
    }
};
...
var REN_SHU = [2, 3, 4];
var JU_SHU = [5, 10, 15];
var JU_SHU_COST = [2, 3, 6];//优惠3天

// 非AA情况下房卡消耗 等于 AA下房卡消耗 * 人数 2,3,4
var JU_SHU_COST_AA = [1, 1, 2]; // AA情况下房卡消耗
...
exports.getConf = function (roomConf, creator) {
    ...
    return {
        ...
        playerCount: !roomConf.numOfPlayers ? 3 : REN_SHU[roomConf.numOfPlayers],   //玩家人数
