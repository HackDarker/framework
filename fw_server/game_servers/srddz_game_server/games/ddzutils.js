...
var Card = function (number) {
    ...
    return {
        type: type1,
        value: type1 == 5 ? weight : value,
        number: number,
        name: type + "_" + value,
        weight: weight,
        voice: voice,
        changeToNumber: changeToNumber,
        isLaizi: type1 == 5 ? true : false,
    };
};
exports.Card = Card;
...
var checkPlane = function (cards) {
    ...
    if (size >= 2 && isSequenceArr(threeList) && size === twosize && isSequenceArr(twoList) && len === size * 3 + twosize * 2) {
        return CardType.c1112223344
    }

    if (size >= 2) {
        var count = 0;
        //old  没有考虑3连 ：带的3张单牌也可能是3张连续的 777888999+444
        ...
        for (let j = 0; j <= Math.floor(size / 2) - 1; j++) {
            for (var i = 2; i <= size; i++) {
                var threeListArr = threeList.slice(j, i);
                if (isSequenceArr(threeListArr)) {
                    if (threeListArr.length > count && threeListArr.length * 4 === len) {
                        count = threeListArr.length;
                    }
                    continue;
                }
                break;
...
var checkPlaneWithLaizi = function (normalCards, laiziCards, cardsValue) {
    ...
    //     return resultTypes;
    // }
    if (totalLen >= 6 && totalLen % 3 == 0) {
        _availableCardTypes.push(CardType.c111222);
    }
    if (totalLen >= 8 && totalLen % 4 == 0) {
        _availableCardTypes.push(CardType.c11122234);
    }
    if (totalLen >= 10 && totalLen % 5 == 0) {
        _availableCardTypes.push(CardType.c1112223344);
    }
    ...
    if (_availableCardTypes.indexOf(CardType.c111222) >= 0) {
        if (laiziLen == 0) {
            ...
            for (i = begin >= 3 ? begin : 3; i < 15; i++) {
                ...
                var canUse = true;
                if (laiziLen == 3) {
                    for (let index = 0; index < _temp_laizi_cards.length - 2; index++) {//飞机中 3王不能替代没有的牌
                        if (Math.floor(_temp_laizi_cards[index].changeToNumber / 100) ===
                            Math.floor(_temp_laizi_cards[index + 1].changeToNumber / 100) &&
                            Math.floor(_temp_laizi_cards[index].changeToNumber / 100) ===
                            Math.floor(_temp_laizi_cards[index + 2].changeToNumber / 100)) {
                            canUse = false;
                        }
                    }
                }
                if (canUse && _temp_three_list.length * 3 == totalLen && isSequenceArr(_temp_three_list.sort(compareTwoNumbers))) {
...
var judgeType = function (list) {
    // console.log('judgeType list', list);
    ...
    if (cardsLen + laiziLen == 1) {
        ...
        if (fourList.length == 0 && threeList.length == 0) {
            ...
            for (i = begin >= 3 ? begin : 3; i < 15; i++) {
                ...
                var canUse = true;
                if (laiziLen == 2) {
                    for (let index = 0; index < _temp_laizi_cards.length - 1; index++) {//连队中 两王不能替代没有的牌
                        if (Math.floor(_temp_laizi_cards[index].changeToNumber / 100) ===
                            Math.floor(_temp_laizi_cards[index + 1].changeToNumber / 100)) {
                            canUse = false;
                        }
                ...
                if (canUse && _temp_two_list.length * 2 == cardsLen + laiziLen && isSequenceArr(_temp_two_list.sort(compareTwoNumbers))) {
...
var isLaizi = function (value, laiziValue) {
    if (laiziValue && laiziValue.length > 0) {
        var card = new Card(value);
        if (card.number == 52 || card.number == 53) {
            return true;
        }
        return false;
    }
...
exports.getPromptCards = function (playerCards, myCards, laiziValue) {
    ...
    var getPairCards = function (normalCards, laiziCards) {
        ...
        for (var i = 0; i < cardArr.length - 1; i++) {
            if (cardArr[i].weight === cardArr[i + 1].weight) {
                ...
                if (cardArr[i].number === cardArr[i + 1].number) {
                    tempSameCardList.push({
                        weight: cardArr[i].weight,
                        cards: [cardArr[i], cardArr[i]]
                    });
                    continue;
                }
                ...
                cardsWeightArr.push(cardArr[i].weight);
        ...
        //两副牌中一样的牌组成的对子
        for (let sameIndex = 0; sameIndex < tempSameCardList.length; sameIndex++) {
            const sameCard = tempSameCardList[sameIndex];
            if (cardsWeightArr.indexOf(sameCard.weight) == -1) {
                cardsArr.push(sameCard);
            }
        }
    ...
    var getFourCards = function (normalCards, laiziCards) {
        ...
        if (laiziCards.length > 2) {
            for (var i = 0; i < laiziCards.length - 2; i++) {
                ...
                var lastLaiziCards = getExpectCards(laiziCards, [laizi1.number, laizi2.number, laizi3.number]);
                var singleCards = getSingleCards(normalCards, lastLaiziCards);
                var singleCardsArr = singleCards.cardsArr;
                for (var j = 0; j < singleCardsArr.length; j++) {
                    if (singleCardsArr[j].weight < 16) {
    ...
    if (playerCardType === CardType.c1) {
        ...
        //#endregion
        for (var i = 0; i < bombs.length; i++) {
            var solfBombWeight = getBombWeight(toCardsArr(bombs[i]));
            if (bombs[i].length > playerCards.length) {
                result.push(bombs[i]);
            } else if (bombs[i].length == playerCards.length && solfBombWeight > playerWeight) {
                result.push(bombs[i]);
            }
        }

        for (var i = 0; i < fourList.length; i++) {//纯炸弹
            var bombWeight = getBombWeight(toCardsArr(countArr[fourList[i]].numbers));
            if (countArr[fourList[i]].count > playerCards.length) {
                result.push(countArr[fourList[i]].numbers);
            } else if (countArr[fourList[i]].count == playerCards.length && bombWeight > playerWeight) {
                result.push(countArr[fourList[i]].numbers);
            }
        }
