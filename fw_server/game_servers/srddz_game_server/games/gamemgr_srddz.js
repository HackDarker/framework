...
const GAMETYPE = consts.GameType.SRDDZ;
...
// //--两幅牌--cheng
// var towPairsCardPool = [
//     0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,                //方块 A - K
//     0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D,                //梅花 A - K
//     0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D,                //红桃 A - K
//     0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D,                //黑桃 A - K
//     0x101, 0x102, 0x103, 0x104, 0x105, 0x106, 0x107, 0x108, 0x109, 0x10A, 0x10B, 0x10C, 0x10D,   //方块 A - K
//     0x111, 0x112, 0x113, 0x114, 0x115, 0x116, 0x117, 0x118, 0x119, 0x11A, 0x11B, 0x11C, 0x11D,   //梅花 A - K
//     0x121, 0x122, 0x123, 0x124, 0x125, 0x126, 0x127, 0x128, 0x129, 0x12A, 0x12B, 0x12C, 0x12D,   //红桃 A - K
//     0x131, 0x132, 0x133, 0x134, 0x135, 0x136, 0x137, 0x138, 0x139, 0x13A, 0x13B, 0x13C, 0x13D,   //黑桃 A - K
// ];

// //--两幅王--cheng
// var towPairsKing = [0x4E, 0x4F,  0x14F, 0x14E];

// 区分下三人还是四人斗地主

//var dealCardCount = 17;// 每个人25张
...
function shuffle(game) {
    ...
    //     if (cards[index] === 52 || cards[index] === 53) {
    //         kings.push(cards.splice(index, 1)[0]);
    //         num++;
    //         if (num == 3) {
    //             break;
    //         }
...
function deal(game) {
    ...
    for (var i = 0; i < game.gameSeats.length; ++i) {
        ...
        holds = game.cards.slice(i * 25, (i + 1) * 25);
        game.gameSeats[i].holds = holds;
        //判断手牌中是否有双王4个2
        var tempCount1 = 0;
        var tempCount2 = 0;
        for (var j = 0; j < 25; j++) {//4个2
            if (holds[j] === 1
                || holds[j] === 14
                || holds[j] === 27
                || holds[j] === 40
        ...
        // }
        if (tempCount2 === 4) {
            game.fourKing = game.gameSeats[i];
        }

    }
    game.dizhuPaiArr = game.cards.slice(100);
...
function dealLZ(game) {
    ...
    game.laiziPaiArr.push(16);
    game.laiziPaiArr.push(17);
}
...
function doGameOver(game, userId, dissolve, tiQian) {
    ...
    var fnNoticeResult = function (isEnd) {
        ...
        if ((isEnd || dissolve) && !roomInfo.conf.bsc) {
            setTimeout(function () {
                fibers(function () {
            ...
            if (roomInfo.conf.bsc && isEnd) {
                roomMgr.detail_match_push(userId, (data) => {
                    console.log('================>>>>>>>游戏结束执行这里2')
    ...
    if (game != null) {
        ...
        if (!dissolve) {
            ...
            if (seatData.isDizhu) {
                ...
                score *= 3;
                var other1 = game.gameSeats[(seatData.seatIndex + 1) % 4];
                var other2 = game.gameSeats[(seatData.seatIndex + 2) % 4];
                var other3 = game.gameSeats[(seatData.seatIndex + 3) % 4];
                if (!tiQian) {
                    seatData.score += score;

                    other1.score -= score / 3;
                    other2.score -= score / 3;
                    other3.score -= score / 3;
                } else {
                    if (game.fourKing == game.gameSeats[seatData.seatIndex]) {
                        seatData.score += score;
                        other1.score -= score / 3;
                        other2.score -= score / 3;
                        other3.score -= score / 3;
...
function beginPlay(game) {
    ...
    game.cards = new Array(108);
    //当前轮到玩家index
    game.turn = parseInt(Math.random() * 4);
    //最终地主获得的3张牌
    game.dizhuPaiArr = [];
    //癞子牌
    game.laiziPaiArr = [];
...
function jiao3Fen(game, score, seatData) {
    ...
    if (score === 3) {
        ...
        for (var j = 0; j < seatData.holds.length; j++) {
            ...
            if (seatData.holds[j] === 52 || seatData.holds[j] === 53) {//4王
                tempCount2++;
            }
        }

        if (tempCount2 === 4) {
            game.fourKing = game.gameSeats[seatData.seatIndex];
        }
...
function buJiaoQiang(game, seatData, score) {
    ...
    //上下家index
    var upIdx = (seatData.seatIndex + 3) % 4;
    var downIdx = (seatData.seatIndex + 1) % 4;
...
exports.chuPai = function (userId, data) {
    ...
    console.log('current out paiArr', paiArr);
    console.log('current out cards', tempArr);
    ...
    if (game.lastPaiData.userId !== userId) {
        ...
        console.log('game.laiziPaiArr', game.laiziPaiArr);
        console.log('验证出牌 resultTypes', resultTypes);
    ...
    if (paiArr.indexOf(52) < 0 && paiArr.indexOf(53) < 0) {//非癞子
...
exports.autoChupai = function (turnSeat, game) {
    //玩家托管，自动打牌
    console.log('玩家 ' + turnSeat.userId + ' 托管，2s后自动打牌');
    setTimeout((turnSeat, game) => {
        ...
        if (turnSeat.userId != game.dizhuId && lastPaiData.userId != turnSeat.userId && lastPaiData.userId != game.dizhuId) {//农民队友
            ...
            if (lastPaiData.userId == turnSeat.userId || lastPaiData.paiArr.length == 0) {//1.上一把是自己出的；2.地主一开始就托管
                var resultCards = ddzutils.getPromptCards([41], mycards, game.laiziPaiArr);
                var mycardType = ddzutils.judgeType(mycards);
                if (ddzutils.availableCardTypes.indexOf(mycardType) != -1 &&
                    mycardType != ddzutils.CardType.c11122234 &&
                    mycardType != ddzutils.CardType.c31 &&
                    mycardType != ddzutils.CardType.c422 &&
                    mycardType != ddzutils.CardType.c123) {
                    resultCards = [mycards];//最后自己出牌  如果剩余牌能组成牌型则全部出完
                }

            } else {
                var resultCards = ddzutils.getPromptCards(lastPaiData.paiArr, mycards, game.laiziPaiArr);
            }
            resultCards.sort((a, b) => {
                return a.length - b.length;
            })
            console.log('autoChupai getPromptCards resultTypes:', resultCards);
            resultCards = resultCards.length == 0 ? [] : resultCards[0];
            if (resultCards.length > 0 && lastPaiData.userId !== turnSeat.userId && lastPaiData.paiArr.length !== 0) {
                var resultTypes = ddzutils.checkCards(game.lastPaiData.paiArr, resultCards, false, game.laiziPaiArr);
                console.log('autoChupai getPromptCards resultTypes:', resultTypes);
                resultCards = resultTypes[0].cardArr;
            }
        }
